...
import (
	...
	"path/filepath"
	"text/template"
	//"github.com/LukaP/bookings/internal/config"
	//"github.com/LukaP/bookings/internal/models"
...
func CreateTemplateCache() (map[string]*template.Template, error) {
	...
	for _, page := range pages {
		...
		if len(matches) > 0 {
			ts, err = ts.ParseGlob(fmt.Sprintf("%s/*.layout.tmpl", PathToTemplates)) // pazi tukaj ne declaras se enkrat ts, err ker sta ze prej
			if err != nil {
		...
		myCache[name] = ts // tko dejansko kreiramo rendered template... ce ni errorjev vstavimo about in home v template cache
